...
class TokokuFragment : BaseFragment<TokokuPresenter, TokokuContract.View>(), TokokuContract.View {
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance() = TokokuFragment()
    }

    override fun createPresenter(): TokokuPresenter {
        return TokokuPresenter(activity as Context, this)
    }
...
//    }
    ...
    override fun setProfile(
        ...
        if (url.isBlank() || url.isEmpty()) {
            Glide.with(requireActivity()).load(R.drawable.logo).transform(CenterCrop(), CircleCrop())
                .into(_view.iv_photo)
            //_view.tv_caption.visibility = View.VISIBLE
        } else {
            Glide.with(requireActivity()).load(url).error(R.drawable.logo).placeholder(R.drawable.logo)
