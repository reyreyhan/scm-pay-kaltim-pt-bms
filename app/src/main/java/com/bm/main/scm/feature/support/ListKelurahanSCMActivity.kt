...
@AndroidEntryPoint
class ListKelurahanSCMActivity : AppCompatActivity(), ListKelurahanAdapter.OnItemClickListener {
    @Inject
    lateinit var apiService: SupportRestInterface
    var disposable: Disposable? = null
    lateinit var adapter: ListKelurahanAdapter
    ...
    private fun initRecyclerView() {
        ...
        adapter = ListKelurahanAdapter(emptyList(), this)
    ...
    override fun onItemClick(item: Kelurahan) {
        setResult(RESULT_OK, Intent().putExtra("kelCode", item.id_kelurahan).putExtra("kelName", item.detail).putExtra("posCode", item.kode_pos))
    ...
    fun loadData() {
        disposable?.dispose()
        disposable = apiService.getKelurahan(createJsonRequestBody())
    ...
    private fun createJsonRequestBody() =
        RequestBody.create(
            ...
            StringJson(this).requestKodePos(intent.getStringExtra("kecCode")))
}