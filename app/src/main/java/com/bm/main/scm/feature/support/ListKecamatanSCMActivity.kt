...
@AndroidEntryPoint
class ListKecamatanSCMActivity : AppCompatActivity(), ListKecamatanAdapter.OnItemClickListener {
    @Inject
    lateinit var apiService: SupportRestInterface
    var disposable: Disposable? = null
    lateinit var adapter: ListKecamatanAdapter
    ...
    private fun initRecyclerView() {
        ...
        adapter = ListKecamatanAdapter(emptyList(), this)
    ...
    override fun onItemClick(item: Kecamatan) {
        setResult(RESULT_OK, Intent().putExtra("kecCode", item.kecamatan_code).putExtra("kecName", item.kecamatan_name))
    ...
    fun loadData() {
        disposable?.dispose()
        disposable = apiService.getKecamatan(createJsonRequestBody())
    ...
    private fun createJsonRequestBody() =
        RequestBody.create(
            ...
            StringJson(this).requestKecamatan(intent.getStringExtra("kabCode")))
}