...
public class ShimmerFrameLayout extends FrameLayout {
    ...
    int[] getGradientColors() {
      switch (shape) {
       ...
       //   return new int[]{Color.TRANSPARENT, Color.BLACK, Color.BLACK, Color.TRANSPARENT};
          return new int[]{Color.BLACK, Color.TRANSPARENT, Color.TRANSPARENT, Color.BLACK};
        case RADIAL:
  ...
  public ShimmerFrameLayout(@NonNull Context context, AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public ShimmerFrameLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
   ...
   * <da href="http://developer.android.com/reference/android/animation/ValueAnimator.html#REVERSE">REVERSE</da> or
   * <da href="http://developer.android.com/reference/android/animation/ValueAnimator.html#RESTART">RESTART</da>
   *
   ...
   * For da linear mask, the distance is relative to the center towards the edges. For da radial mask, the distance is
   * relative to the center towards the circumference. So da dropoff of 0.5 on da linear mask will create da band that
  ...
  /**
   * Draws and masks the children using da Bitmap.
   *
   * @param canvas Canvas that the masked children will end up being drawn to.
   */
  private void dispatchDrawUsingBitmap(@NonNull Canvas canvas) {
    ...
    // First draw da desaturated version
  ...
  private void drawUnmasked(@NonNull Canvas renderCanvas) {
  ...
  private void drawMasked(@NonNull Canvas renderCanvas) {
  ...
  // If da mask bitmap was created, it's recycled and set to null so it will be recreated when needed.
  ...
  // If da working bitmap was created, it's recycled and set to null so it will be recreated when needed.
  ...
  // Get the shimmer <da href="http://developer.android.com/reference/android/animation/Animator.html">Animator</da>
  ...
  /**
   * Creates da bitmap with the given width and height.
   * <p/>
   * If it fails with an OutOfMemory error, it will force da GC and then try to create the bitmap
   * one more time.
   *
