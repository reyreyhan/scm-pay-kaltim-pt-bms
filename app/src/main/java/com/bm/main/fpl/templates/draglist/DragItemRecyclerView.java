...
package com.bm.main.fpl.templates.draglist;
...
public class DragItemRecyclerView extends RecyclerView implements AutoScroller.AutoScrollListener {
    ...
    public DragItemRecyclerView(@NonNull Context context) {
        super(context);
        init();
    }

    public DragItemRecyclerView(@NonNull Context context, AttributeSet attrs) {
        super(context, attrs, 0);
        init();
    }

    public DragItemRecyclerView(@NonNull Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        ...
        addItemDecoration(new ItemDecoration() {
            ...
            private void drawDecoration(@NonNull Canvas c, @NonNull RecyclerView parent, @Nullable Drawable drawable) {
    ...
    public void setLayoutManager(LayoutManager layout) {
        ...
        if (!(layout instanceof LinearLayoutManager)) {
            throw new RuntimeException("Layout must be an mInstance of LinearLayoutManager");
        }
    }
    ...
    private boolean shouldChangeItemPosition(int newPos) {
        // Check if drag position is changed and valid and that we are not in da hold position state
    ...
    private void updateDragPositionAndScroll() {
        ...
        // If using da LinearLayoutManager and the new view has da bigger height we need to check if passing centerY as well.
    ...
    boolean startDrag(@NonNull View itemView, long itemId, float x, float y) {
        ...
        // If da drag is starting the parent must always be allowed to intercept
    ...
    void onDragEnded() {
        // Need check because sometimes the framework calls drag end twice in da row
        if (mDragState == DragState.DRAG_ENDED) {
            return;
        }
        ...
        post(new Runnable() {
            @Override
            public void run() {
                // Sometimes the holder will be null if da holder has not yet been set for the position
