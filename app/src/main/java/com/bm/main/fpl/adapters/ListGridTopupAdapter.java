...
public class ListGridTopupAdapter extends RecyclerView.Adapter<ListGridTopupAdapter.ViewHolder> implements Filterable {
    private static final String TAG = ListGridTopupAdapter.class.getSimpleName();
    private ArrayList<EmoneyModel.Response_value> data = new ArrayList<>();
    private ArrayList<EmoneyModel.Response_value> filterData = new ArrayList<>();
    private Context context;
    private OnClickProduk listener;
    private int flag=0;

    public ListGridTopupAdapter(ArrayList<EmoneyModel.Response_value> data, Context context, OnClickProduk listener) {
        this.data = data;
    ...
    @NonNull
    @Override
    public ListGridTopupAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View v;
...
//        );
    ...
    @Override
    public void onBindViewHolder(@NonNull final ListGridTopupAdapter.ViewHolder viewHolder, int i) {
        final EmoneyModel.Response_value produk = filterData.get(i);
...
//        });
        ...
        Glide.with(context).asBitmap().load(produk.getGame_url()).encodeFormat(Bitmap.CompressFormat.WEBP).encodeQuality(50).override(50,50).diskCacheStrategy(DiskCacheStrategy.NONE).into(new BitmapImageViewTarget(viewHolder.imageViewProduk) {
            @Override
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(@NonNull CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    ArrayList<EmoneyModel.Response_value> filteredList = new ArrayList<>();

                    for (EmoneyModel.Response_value produk : data) {
            ...
            protected void publishResults(CharSequence charSequence, @NonNull FilterResults filterResults) {
                filterData = (ArrayList<EmoneyModel.Response_value>) filterResults.values;
...
//String url;
    ...
    public interface OnClickProduk {
        void onClickProduk(EmoneyModel.Response_value produk);
    }
}
