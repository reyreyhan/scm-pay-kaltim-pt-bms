...
public class ListGridMerchantAdapter extends RecyclerView.Adapter<ListGridMerchantAdapter.ViewHolder> implements Filterable {
    private ArrayList<MerchantModel.Response_value>data ;//= new ArrayList<>();
    private ArrayList<MerchantModel.Response_value>filterData;// = new ArrayList<>();
    private Context context;
    public OnClickMerchant listener;
    SparseBooleanArray selectedItems;

    public ListGridMerchantAdapter(ArrayList<MerchantModel.Response_value> data, Context context, OnClickMerchant listener){
        this.data = data;
    ...
    @NonNull
    @Override
    public ListGridMerchantAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View v;
        v = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.list_bank_grid,viewGroup,false);
...
//        );
    ...
    @Override
    public void onBindViewHolder(@NonNull final ListGridMerchantAdapter.ViewHolder viewHolder, final int i) {
        final MerchantModel.Response_value produk = filterData.get(i);
...
//
//                Glide.with(context).load( R.mipmap.ic_launcher).dontAnimate().diskCacheStrategy(DiskCacheStrategy.ALL).into(viewHolder.imageViewProduk);
//
//                viewHolder.avi.setVisibility(View.GONE);
//                return false;
//            }
...
//
        ...
        Glide.with(context).asBitmap().load(produk.getProduct_url()).encodeFormat(Bitmap.CompressFormat.WEBP).encodeQuality(50).diskCacheStrategy(DiskCacheStrategy.NONE).override(50,50).into(new BitmapImageViewTarget(viewHolder.imageViewProduk) {
            @Override
...
//                }
               // notifyDataSetChanged();
                ...
                if (adapterPos != RecyclerView.NO_POSITION) {
                    if (listener != null) {
                        ...
                        listener.OnClickGridMerchant(produk,adapterPos);
                    }
                }
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(@NonNull CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    ArrayList<MerchantModel.Response_value> filteredList = new ArrayList<>();

                    for (MerchantModel.Response_value produk : data) {

                        if (produk.getMerchant_code().toLowerCase().contains(charString) || produk.getMerchant_name().toLowerCase().contains(charString) ) {
            ...
            protected void publishResults(CharSequence charSequence, @NonNull FilterResults filterResults) {
                filterData = (ArrayList<MerchantModel.Response_value>) filterResults.values;
                notifyDataSetChanged();
            }
        };
    }
    ...
    public interface OnClickMerchant {
        void OnClickGridMerchant(MerchantModel.Response_value produk, int adapterPos);
    }
}
