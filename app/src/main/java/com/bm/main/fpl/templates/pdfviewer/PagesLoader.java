...
import com.bm.main.fpl.templates.pdfviewer.PDFView;
...
import static com.bm.main.fpl.templates.pdfviewer.util.Constants.Cache.CACHE_SIZE;
import static com.bm.main.fpl.templates.pdfviewer.util.Constants.PRELOAD_OFFSET;
...
class PagesLoader {
    ...
    PagesLoader(@NonNull PDFView pdfView) {
        this.pdfView = pdfView;
        this.preloadOffset = Util.getDP(pdfView.getContext(), PRELOAD_OFFSET);
    }

    private void getPageColsRows(@NonNull GridSize grid, int pageIndex) {
    ...
    @NonNull
    private Holder getPageAndCoordsByOffset(@NonNull Holder holder, @NonNull GridSize grid, float localXOffset,
    ...
    private void calculatePartSize(@NonNull GridSize grid) {
     ...
     */
    private int loadWholePage(int page, @NonNull GridSize grid, int nbOfPartsLoadable) {
     ...
     * @return loaded parts count
     */
    private int loadPageCenter(@NonNull Holder firstHolder, @NonNull Holder lastHolder, @NonNull GridSize grid, int nbOfPartsLoadable) {
        calculatePartSize(grid);
     ...
     * @return loaded parts count
     */
    private int loadPageEnd(@NonNull Holder holder, @NonNull GridSize grid, int nbOfPartsLoadable) {
        calculatePartSize(grid);
        if (pdfView.isSwipeVertical()) {
     ...
     * @return loaded parts count
     */
    private int loadPageStart(@NonNull Holder holder, @NonNull GridSize grid, int nbOfPartsLoadable) {
        calculatePartSize(grid);
        if (pdfView.isSwipeVertical()) {
