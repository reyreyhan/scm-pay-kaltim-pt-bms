package com.bm.main.fpl.templates.Radar;
...
public class RadarScanView extends View
{
    ...
    private int circleColor = Color.parseColor("#B3E5FC");
   // private int radarColor = Color.parseColor("#99a2a2a2");
    private int radarColor = Color.parseColor("#B3E5FC");
//    private int tailColor = Color.parseColor("#50aaaaaa");
    private int tailColor = Color.parseColor("#B3E5FC");
    ...
    private Runnable run = new Runnable()
    {
        ...
        public void run()
        {
            ...
            postInvalidate();
            handler.postDelayed(run, 7);
        }
    };

    public RadarScanView(@NonNull Context context)
    {
        super(context);
        init(null, context);
    }

    public RadarScanView(@NonNull Context context, AttributeSet attrs)
    {
        super(context, attrs);
        init(attrs, context);
    }

    public RadarScanView(@NonNull Context context, AttributeSet attrs, int defStyleAttr)
    {
    ...
    private void init(@Nullable AttributeSet attrs, @NonNull Context context)
    {
        if (attrs != null)
        {
...
//        canvas.drawCircle(centerX, centerY, radarRadius / 2, mPaintCircle);
        canvas.drawCircle(centerX, centerY, 3 * radarRadius / 7, mPaintCircle);

        //设置颜色渐变从透明到不透明
             //  Shader shader = new SweepGradient(centerX, centerY, radarColor, tailColor);
    ...
    private int dip2px(@NonNull Context context, float dipValue)
    {
        final float scale = context.getResources().getDisplayMetrics().density;
        return (int) (dipValue * scale + 0.1f);
    }

    private int px2dip(@NonNull Context context, float pxValue)
    {
        final float scale = context.getResources().getDisplayMetrics().density;
