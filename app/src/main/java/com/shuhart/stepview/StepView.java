...
public class StepView extends View {
    ...
    public StepView(@NonNull Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.sv_stepViewStyle);
    }

    public StepView(@NonNull Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void applyStyles(@NonNull Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (fontId != 0) {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                ...
                typeface = getFont(context,fontId);
            }
            if (typeface != null) {
    ...
    public void setOnStepClickListener(@Nullable OnStepClickListener listener) {
        setClickable(listener != null);
        onStepClickListener = listener;
    }

    public void setSteps(@NonNull List<String> steps) {
        stepsNumber = 0;
        displayMode = DISPLAY_MODE_WITH_TEXT;
        this.steps.clear();
    ...
    private void measureConstraints(int width) {
        ...
        constraints[0] = (float) width / getStepCount();
    ...
    private int getMaxLineWidth(@NonNull StaticLayout layout) {
    ...
    private <T> T last(@NonNull T[] array) {
    ...
    private void drawStep(@NonNull Canvas canvas, int step, int circleCenterX, int circleCenterY) {
    ...
    private void drawNumber(@NonNull Canvas canvas, @NonNull String number, int circleCenterX, @NonNull Paint paint) {
    ...
    private void drawText(@NonNull Canvas canvas, @NonNull String text, int y, int step) {
        if (text.isEmpty()) {
            return;
        }
    ...
    private void drawCheckMark(@NonNull Canvas canvas, int circleCenterX, int circleCenterY) {
    ...
    private void drawLine(@NonNull Canvas canvas, int startX, int endX, int centerY, boolean highlight) {
        if (highlight) {
